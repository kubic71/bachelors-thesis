...
CSV_HEADER: str = "img_fn\thuman_readable_label\ttarget_loss\ttop_organism_label\ttop_object_label\tdist\tsurrogate\ttarget\n"
...
class SimpleTransferRegime(AttackRegime):
    ...
    def run(self) -> None:
        ...
        for img_paths, imgs, labels, human_readable_labels in DataLoader.create_batches(
            ...
            self.save_results(results, self.regime_config.results_dir, self.n_successful, self.total)
    ...
    def save_results(self, results: BatchTransferResult, results_dir: str, n_successful: int, total: int) -> None:

        with open(results_dir + "/" + RESULTS_FILENAME, "a") as f:
            for r in results.items():
                ...
                if (not self.regime_config.save_only_successful_images) or r.target_loss < 0:
                    self.save_adv_img(r.x_adv.detach().cpu().numpy().transpose(1, 2, 0), r.img_fn, results_dir + "/adv_examples") 
    ...
    def write_summary(self, n_successful: int, n_total: int, destination_dir: str, surrogate_name: str, target_name: str) -> None:
        summary = f"trasfer attack from {surrogate_name} to {target_name}\nsuccessfully transferred / total = {n_successful}/{n_total} = {(n_successful/n_total*100):.2f}%"
        logger.info(summary)
