...
def pseudo_gaussian_pert_rectangles(x: np.ndarray, y:np.ndarray) -> np.ndarray:
    delta = np.zeros([x, y])
    x_c, y_c = x // 2 + 1, y // 2 + 1
...
class SquareL2AttackAlgorithm(BlackBoxIterativeAlgorithm):
    def __init__(self, image: np.ndarray, loss_fn: LossCallCounter, epsilon: float = 20, n_iters: int = 1000, p_init: float = 0.2):
    ...
    def run(self) -> Generator[np.ndarray, None, None]:
        ...
        s = h // 5
        logger.debug('Initial square side={} for bumps'.format(s))

        # Throws errors for some non-square images
        # sp_init = (h - s * 5) // 2
        ...
        loss_min = np.array([self.loss_fn(x_best[0].transpose(1, 2, 0))])
        margin_min = loss_min
        yield x_best[0]
        ...
        for i_iter in range(n_iters):
            ...
            norms_window_2 = np.sqrt(
                np.sum(delta_curr[:, :, center_h_2:center_h_2 + s2, center_w_2:center_w_2 + s2]**2,
                       axis=(-2, -1),
                       keepdims=True))
            ...
            loss = np.array([self.loss_fn(x_new[0].transpose(1, 2, 0))])
            margin = loss
            ...
            yield x_best[0].transpose(1, 2, 0)
...
def square_attack_linf(model, x, y, corr_classified, eps, n_iters, p_init, metrics_path, targeted, loss_type):
    ...
    for i_iter in range(n_iters - 1):
        ...
        for i_img in range(x_best_curr.shape[0]):
            ...
            while np.sum(
                    np.abs(
                        np.clip(x_curr_window +
                                deltas[i_img, :, center_h:center_h + s, center_w:center_w + s], min_val, max_val) -
        ...
        if (i_iter <= 500 and i_iter % 20 == 0) or (i_iter > 100
                                                    and i_iter % 50 == 0) or i_iter + 1 == n_iters or acc == 0:
            np.save(metrics_path, metrics)
        if acc == 0:
            break
...
if __name__ == '__main__':
    ...
    if args.model == "gvision":
        labelset = ["cat"]
        model = models.GVisionModel(exp_name=f"eps={args.eps}_p={args.p}_attack={args.attack}_niters={args.n_iter}",
                                    correct_labelset=labelset,
                                    save_location="l2_cat")
        y_test = np.array([0])
    ...
    n_queries, x_adv = square_attack(model, x_test, y_target_onehot, corr_classified, args.eps, args.n_iter, args.p,
                                     metrics_path, args.targeted, args.loss)
"""
