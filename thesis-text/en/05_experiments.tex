...
Figure \ref{fig:apgd_noise_augment} shows that in the low iteration APGD regime the optimal Guassian-noise augmentation std is somewhere near $\sigma=18$. 


Because the surrogate is stochastic and during each forward pass applies a different transformation $t \sim T$, we estimate estimate the true gradient in a Expectation over Transformation (EoT - \cite{athalye2018synthesizing}) style:


It also shows that given the same whitebox query budget, it pays off to get a better quality gradient estimate.

This is in agreement with \cite{Wu2020TowardsUA}, who find the distortion level $\sigma = 15$ to be performing the best.
...
Figure \ref{fig:apgd_box_blur_sample} shows the effect of increasing the kernel size of the box-blur on the frequency of the adversarial pertubation.
